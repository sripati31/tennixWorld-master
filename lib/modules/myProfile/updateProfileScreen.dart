...
import '../../constant/sharedPreferences.dart';
...
class _UpdateProfileScreenState extends State<UpdateProfileScreen> {
  ...
  void initState() {
    ...
    if (loginUserData.dob != null && loginUserData.dob != '') date = DateFormat.yMd().parse(loginUserData.dob!);
    slectedState = StateList(name: 'Select your state');
    stateList.insert(0, slectedState);
    slectedCity = CityList(name: 'Select your city');
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        InkWell(
                          radius: 100,
                          onTap: () async {
                            _bottomNavProvider.hideTabBarStatus();

                            Map<String, dynamic>? result = await Navigator.push(context, MaterialPageRoute(builder: (builder) => const CustomImagePickerScreen()));
                            // Map<String,dynamic> ?result = await pushNewScreenWithRouteSettings(
                            //   context,
                            //   settings: RouteSettings(name: Routes.CameraPickerRoute),
                            //   screen: CustomImagePickerScreen(),
                  ...
                  children: <Widget>[
                    Padding(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          fontFamily: 'Poppins',
                          fontSize: DateFormat.yMd().format(date) != DateFormat.yMd().format(DateTime.now()) ? AppConstant.SIZE_TITLE12 : AppConstant.SIZE_TITLE16,
  ...
  Future<void> _updateUser() async {
    ...
    API_STRUCTURE apiObject = API_STRUCTURE(
        context: context,
        apiName: ApiConstant.updateUser,
    ...
    if (apiResponse.containsKey(API_RESPONSE.SUCCESS)) {
      ...
      // });
      //
      // CustomToast.showToast(message: 'Succsessfull Update Profile');
      // Navigator.pop(context);
      //
      // setState(() {
